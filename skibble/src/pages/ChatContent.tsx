...
import type { ChatUser } from "../appState/state";
...
function ChatContent({ user_id }: ChatContentProps) {
  ...
  const existingUser = store.chats[user_id];
  useEffect(() => {
    if (existingUser) {
  ...
  if (!store.token || !userData.username) {
          ...
          <div className="chat-profile">
            <div className="chat-avatar image-wrapper">
              <img
                src={
                  userData.avatar
                    ? `${import.meta.env.VITE_API_BASE_URL}/file/stream/${
                        userData.avatar
                      }`
                    : "#"
                }
